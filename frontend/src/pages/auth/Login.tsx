...
export default function LoginPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setUser(me.data)
      toast.success(t("Xush kelibsiz, {name}!", { name: me.data.first_name || me.data.username }))
      navigate("/", { replace: true })
    } catch (err) {
      setError(getErrorMessage(err, t("Login yoki parol xato")))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-ink-950 text-white">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1280px] flex-col px-6 py-8 sm:px-10">
        <header className="flex items-center justify-between">
          <Logo />
          <span className="eyebrow hidden text-ink-400 sm:block">{t("Maktab boshqaruv platformasi")}</span>
        </header>
        ...
        <main className="flex flex-1 items-center py-16">
          <div className="w-full max-w-[520px]">
            ...
            >
              {t("Davomat, baholar, vazifalar va muloqot — o'quvchi, o'qituvchi va ota-onalar uchun yagona tizim.")}
            </motion.p>
        ...
        <footer className="flex flex-wrap items-end justify-between gap-6 border-t border-ink-800 pt-6">
          <div>
            <p className="eyebrow mb-3 text-ink-400">{t("Biz bilan bog'laning")}</p>
